...
import { useClassDetail, type ClassComponent } from '@renderer/hooks/use-data'
...
export function ClassCard({ classId, onClose }: ClassCardProps): React.JSX.Element {
  ...
  if (isLoading)
    return <div style={{ padding: 12, color: 'var(--cic-cyan-dim)' }}>Loading class data...</div>
  if (error || !data)
    return <div style={{ padding: 12, color: 'var(--cic-red)' }}>Failed to load class</div>
  ...
  const totalHTK =
    comps.reduce((sum, comp) => sum + comp.HTK * Math.floor(comp.NumComponent), 0) +
    c.ArmourThickness * c.ArmourWidth
  ...
  const other = comps.filter(
    (x) =>
      !engines.includes(x) &&
      !jumpDrives.includes(x) &&
      !weapons.includes(x) &&
      !fireControls.includes(x) &&
      !sensors.includes(x) &&
      !shields.includes(x) &&
      !hangars.includes(x)
  )
  ...
  const s = (v: string | number): React.JSX.Element => (
    <span style={{ color: 'var(--foreground)' }}>{v}</span>
  )
  const a = (v: string | number): React.JSX.Element => (
    <span style={{ color: 'var(--cic-cyan)' }}>{v}</span>
  )
  ...
  return (
    <div style={{ fontSize: 11 }}>
      <div className="flex items-start justify-between" style={{ marginBottom: 8 }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--cic-cyan)' }}>
            {c.ClassName}
          </div>
      ...
      >
        <div>
          {a(c.ClassName)} class — {s(Math.round(c.Tonnage).toLocaleString())} tons — {s(c.Crew)}{' '}
          Crew — {s(Math.round(c.BuildPointCost).toLocaleString())} BP — TCS {s(tcs)} TH{' '}
          {s(c.EnginePower)} EM {s(c.ShieldStrength > 0 ? c.ShieldStrength : 0)}
        </div>
        <div>
          {s(c.MaxSpeed)} km/s
          {c.JumpDistance > 0 && <> — JR {s(`4-${c.JumpDistance}`)}</>} — Armour{' '}
          {s(`${c.ArmourThickness}-${c.ArmourWidth}`)} — Shields {s(`${c.ShieldStrength}-0`)} — HTK{' '}
          {s(totalHTK)} — Sensors{' '}
          {s(`${c.PassiveSensorStrength}/${Math.round(c.EMSensorStrength * 10) / 10}/0/0`)} — DCR{' '}
          {s(c.DCRating)}
        </div>
        <div>
          Control Rating {s(c.ControlRating)} — {stations.join('   ')} — Deploy:{' '}
          {s(c.PlannedDeployment)} months
        ...
        {hangars.length > 0 && (
          <div>
            ...
            {s(
              hangars
                .reduce((sum, h) => sum + h.Tons * Math.floor(h.NumComponent), 0)
                .toLocaleString()
            )}{' '}
            tons
        ...
        <CompSection
          ...
          extra={
            <div style={{ color: 'var(--cic-cyan-dim)', marginTop: 2, fontSize: 10 }}>
              Fuel {c.FuelCapacity.toLocaleString()} L — Range {rangeBkm.toFixed(1)}B km (
    ...
    <div style={{ marginBottom: 8 }}>
      ...
      {items.map((comp, i) => (
        ...
        >
          <span>
            {comp.Name}
            {Math.floor(comp.NumComponent) > 1 && (
              <span style={{ color: 'var(--cic-cyan-dim)' }}>
                {' '}
                (×{Math.floor(comp.NumComponent)})
              </span>
            )}
          </span>
          <span style={{ color: 'var(--cic-cyan-dim)', fontSize: 9 }}>
            {comp.Tons}t{comp.HTK > 0 && <> HTK {comp.HTK}</>}

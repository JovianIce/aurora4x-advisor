...
function loadMineralState(): Record<string, unknown> {
  try {
    return JSON.parse(localStorage.getItem(MINERAL_STORAGE_KEY) || '{}')
  } catch {
    return {}
  }
}
...
export function MineralsTab({ active = true }: { active?: boolean } = {}): React.JSX.Element {
  ...
  const [resolution, setResolutionRaw] = useState<Resolution>(
    (saved.resolution as Resolution) || 'monthly'
  )
  const [populationId, setPopulationIdRaw] = useState<number | null>(
    (saved.populationId as number) ?? null
  )
  const [enabledMinerals, setEnabledMinerals] = useState<Set<string>>(new Set(MINERALS))
  const [selectedMineral, setSelectedMineralRaw] = useState<string | null>(
    (saved.selectedMineral as string) || null
  )

  const setResolution = (v: Resolution): void => {
    setResolutionRaw(v)
    saveMineralState({ resolution: v })
  }
  const setPopulationId = (v: number | null): void => {
    setPopulationIdRaw(v)
    saveMineralState({ populationId: v })
  }
  const setSelectedMineral = (v: string | null): void => {
    setSelectedMineralRaw(v)
    saveMineralState({ selectedMineral: v })
  }

  const { data: totalsData } = useMineralTotals(active)
  const { data: historyData, isFetching: historyLoading } = useMineralHistory(
    resolution,
    populationId,
    active
  )
  ...
  return (
    <div style={{ padding: 12, overflow: 'auto', height: '100%' }}>
      ...
      <div className="flex items-center justify-between" style={{ marginBottom: 12 }}>
        <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--cic-cyan)' }}>
          Mineral Insights
        </span>
      ...
      {selectedMineral && (
        <div className="grid grid-cols-2 gap-4" style={{ marginBottom: 16 }}>
          <div>
            ...
            {totalsData && (
              <LocationBreakdown mineral={selectedMineral} byColony={totalsData.byColony} />
            )}
          </div>
          <div>
...
function TotalsBar({
  ...
  return (
    <div
      style={{ display: 'grid', gridTemplateColumns: `repeat(${MINERALS.length}, 1fr)`, gap: 2 }}
    >
      {MINERALS.map((m) => {
        ...
        return (
          ...
          >
            <div
              style={{ fontSize: 9, color: MINERAL_COLORS[m], fontWeight: 600, marginBottom: 2 }}
            >
              {m.slice(0, 4)}
            </div>
  ...
  return (
    <ResponsiveContainer width="100%" height={320}>
      ...
      >
        ...
        <XAxis
          dataKey="date"
          tick={{ fontSize: 9, fill: 'var(--cic-cyan-dim)' }}
        ...
        <Tooltip
          ...
          formatter={(value, name) => [`${Number(value).toLocaleString()} L`, String(name)]}
        />
        <Legend
          ...
          onMouseEnter={(e) =>
            setHoveredMineral(
              String(
                (e as { dataKey?: string; value?: string }).dataKey ||
                  (e as { dataKey?: string; value?: string }).value
              )
            )
          }
          onMouseLeave={() => setHoveredMineral(null)}
...
function LocationBreakdown({
  ...
  const sorted = byColony
    .map((c) => ({ ...c, amount: c.minerals[mineral] || 0 }))
    .filter((c) => c.amount > 0)
    .sort((a, b) => b.amount - a.amount)
  if (sorted.length === 0)
    return <div style={{ fontSize: 10, color: 'var(--cic-cyan-dim)' }}>No stockpiles</div>
      ...
      <tbody>
        {sorted.map((c) => (
          <tr key={c.populationId}>
            <td style={td}>
              {c.name}
              {c.system && <span style={{ color: 'var(--cic-cyan-dim)' }}> ({c.system})</span>}
            </td>
              ...
              >
                <div
                  style={{
                    ...
                    height: '100%',
                    background: MINERAL_COLORS[mineral],
                    borderRadius: 999
                  }}
                />
              </div>
...
function BreakdownChart({ data }: { data: MineralBreakdownResponse }): React.JSX.Element {
  if (data.series.length === 0)
    return <div style={{ fontSize: 10, color: 'var(--cic-cyan-dim)' }}>No data</div>
    ...
    <ResponsiveContainer width="100%" height={260}>
      ...
      >
        ...
        <Legend
          ...
          formatter={(v: string) => (v.startsWith('-') ? v.slice(1) : v)}
        />
        {[...incomeKeys].map((k, i) => (
          <Bar
            key={k}

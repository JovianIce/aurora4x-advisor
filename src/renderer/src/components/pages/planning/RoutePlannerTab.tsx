...
export function RoutePlannerTab({
  ...
  initialClassId,
  initialFromSystem
}: RoutePlannerProps & { active?: boolean } = {}): React.JSX.Element {
  ...
  const [mode, setModeRaw] = useState<Mode>(
    initialFleetId ? 'fleet' : initialClassId ? 'class' : (saved.mode as Mode) || 'fleet'
  )
  const [classId, setClassIdRaw] = useState<number>(
    initialClassId || (saved.classId as number) || 0
  )
  const [fleetId, setFleetIdRaw] = useState<number>(
    initialFleetId || (saved.fleetId as number) || 0
  )
  const [startSystemId, setStartSystemIdRaw] = useState<number>(
    initialFromSystem || (saved.startSystemId as number) || 0
  )
  const [endSystemId, setEndSystemIdRaw] = useState<number>((saved.endSystemId as number) || 21625)

  const setMode = (v: Mode): void => {
    setModeRaw(v)
    saveRouteState({ mode: v })
  }
  const setClassId = (v: number): void => {
    setClassIdRaw(v)
    saveRouteState({ classId: v })
  }
  const setFleetId = (v: number): void => {
    setFleetIdRaw(v)
    saveRouteState({ fleetId: v })
  }
  const setStartSystemId = (v: number): void => {
    setStartSystemIdRaw(v)
    saveRouteState({ startSystemId: v })
  }
  const setEndSystemId = (v: number): void => {
    setEndSystemIdRaw(v)
    saveRouteState({ endSystemId: v })
  }
  ...
  const canCompute =
    mode === 'class'
      ? classId > 0 && startSystemId > 0 && endSystemId > 0
      : fleetId > 0 && endSystemId > 0
  ...
  if (dataLoading) {
    return (
      <div
        ...
        style={{ color: 'var(--cic-cyan-dim)', fontSize: 11 }}
      >
        Loading route data...
      </div>
    )
  ...
  return (
    <div className="flex h-full">
      ...
      >
        <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--cic-cyan)', marginBottom: 12 }}>
          Route Planner
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    {ja.status === 'covered' && (
                      ...
                      >
                        {ja.shipsWithoutJD.length} ship{ja.shipsWithoutJD.length !== 1 ? 's' : ''}{' '}
                        without jump drive
                        {ja.milTender && (
                          <>
                            {' '}
                            | has mil squad jump: {ja.milTender.shipName} (
                            {ja.milTender.maxTonnage.toLocaleString()}t)
                          </>
                        )}
                        {ja.commTender && (
                          <>
                            {' '}
                            | has comm squad jump: {ja.commTender.shipName} (
                            {ja.commTender.maxTonnage.toLocaleString()}t)
                          </>
                        )}
                      </div>
                    )}
                    {ja.status === 'warning' && (
                      ...
                      >
                        ⚠ {ja.uncoveredShips.length} ship{ja.uncoveredShips.length !== 1 ? 's' : ''}{' '}
                        cannot jump:
                        {ja.uncoveredShips.map((s, i) => (
                          <span key={i}>
                            {' '}
                            {s.shipName} ({s.reason}){i < ja.uncoveredShips.length - 1 ? ',' : ''}
                          </span>
                        ))}
                      </div>
                    )}
                    ...
                    <div style={{ maxHeight: 200, overflow: 'auto', padding: '4px 8px' }}>
                      {selectedFleet.ships.map((s) => {
                        ...
                        const tons =
                          s.tonnage >= 1000 ? `${(s.tonnage / 1000).toFixed(1)}kt` : `${s.tonnage}t`
                        const jdTip = s.jumpDriveInfo
                          ? `${s.jumpDriveInfo.type}, ${s.jumpDriveInfo.maxTonnage.toLocaleString()}t max` +
                            (s.jumpDriveInfo.squadMax > 0
                              ? `, squad ${s.jumpDriveInfo.squadMax}`
                              : '') +
              ...
              {intermediates.map((wp, i) => (
                <div key={i} className="flex gap-1" style={{ marginBottom: 2 }}>
                  ...
                  >
                    ...
                    {systems.map((w) => (
                      <option key={w.systemId} value={w.systemId}>
                        {w.systemName}
                      </option>
                    ))}
                  </select>
                  <button
                    onClick={() => setIntermediates(intermediates.filter((_, j) => j !== i))}
                    className="cursor-pointer"
                    style={{
                      border: 'none',
              ...
              {fleetWaypoints.map((wp, i) => (
                <div key={i} className="flex gap-1 items-center" style={{ marginBottom: 2 }}>
                  <select
                    value={wp.systemId}
                    onChange={(e) => {
                      const next = [...fleetWaypoints]
                      next[i] = { ...next[i], systemId: Number(e.target.value) }
                      setFleetWaypoints(next)
                    }}
                    style={{ ...ss, flex: 1 }}
                  >
                    <option value={0}>Select...</option>
                  ...
                  <label
                    ...
                    style={{ fontSize: 9, color: 'var(--cic-cyan-dim)', whiteSpace: 'nowrap' }}
                  >
                    <input
                      type="checkbox"
                      checked={wp.refuel}
                      onChange={(e) => {
                        const next = [...fleetWaypoints]
                        next[i] = { ...next[i], refuel: e.target.checked }
                        setFleetWaypoints(next)
                      }}
                    />
                    Refuel
                  ...
                  <button
                    onClick={() => setFleetWaypoints(fleetWaypoints.filter((_, j) => j !== i))}
                    className="cursor-pointer"
                    style={{
                      border: 'none',
                      background: 'transparent',
        ...
        <button
          onClick={() =>
            mode === 'class'
              ? setIntermediates([...intermediates, 0])
              : setFleetWaypoints([...fleetWaypoints, { systemId: 0, refuel: false }])
          }
          className="cursor-pointer"
          style={{
            fontSize: 10,
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <SaveRouteButton
            ...
            waypoints={intermediates
              .filter(Boolean)
              .map((id) => ({ systemId: id, systemName: sysName(id) }))}
      ...
      <div className="flex-1 overflow-auto" style={{ padding: 12 }}>
        ...
        {mode === 'class' && classResult && (
          <ClassRouteView result={classResult} gameDate={dateData ?? null} />
        )}
        {mode === 'fleet' && fleetResult && (
          <FleetRouteView result={fleetResult} gameDate={dateData ?? null} />
        )}
        ...
        {!classResult && !fleetResult && !error && (
          ...
          >
            {mode === 'fleet'
              ? 'Select a fleet and destination, then compute.'
              : 'Select a class and waypoints, then compute.'}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-2" style={{ marginBottom: 12 }}>
        <InfoCard
          label="Travel Time"
          value={`${result.totalTravelDays} days`}
          sub={arrivalDate ? `Arrives ${arrivalDate}` : undefined}
        />
        <InfoCard label="Distance" value={formatDistance(result.totalDistanceKm)} />
        <InfoCard
          label="Fuel Burn"
          value={`${result.totalFuelBurn.toLocaleString()} L`}
          sub={`of ${result.fuelCapacity.toLocaleString()} L capacity`}
          warn={!result.sufficient}
          crit={!result.sufficient}
        />
        <InfoCard
          label="Fuel Remaining"
          value={`${result.fuelRemaining.toLocaleString()} L`}
          sub={
            result.sufficient
              ? `${Math.round((result.fuelRemaining / result.fuelCapacity) * 100)}% remaining`
              : 'INSUFFICIENT'
          }
          crit={!result.sufficient}
          warn={result.fuelRemaining < result.fuelCapacity * 0.25}
        />
      </div>

      <SectionHeader>Route Legs</SectionHeader>
      <LegTable
        legs={result.legs.map((l) => ({ ...l, fuelBurn: l.fuelBurn }))}
        gameDate={gameDate}
      />
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-2" style={{ marginBottom: 12 }}>
        ...
        <InfoCard
          label="Fleet Speed"
          value={`${result.fleetSpeed} km/s`}
          sub={`Limited by ${result.speedLimitedBy}`}
        />
        <InfoCard
          label="Bottleneck"
          value={result.bottleneck ? result.bottleneck.name : 'None'}
          sub={
            result.bottleneck
              ? `Short ${result.bottleneck.shortfall.toLocaleString()} L on leg ${result.bottleneck.runsOutOnLeg + 1}`
              : 'All ships sufficient'
          }
          crit={!!result.bottleneck}
        />
      </div>
      ...
      {result.tankerInFleet && (
        <div
          style={{
            ...
            border: '1px solid var(--cic-amber)',
            borderRadius: 4,
            fontSize: 10
          }}
        >
          <span style={{ color: 'var(--cic-amber)', fontWeight: 600 }}>Tanker: </span>
          {result.tankerInFleet.name}: {result.tankerInFleet.fuelRemaining.toLocaleString()} L at
          destination (of {result.tankerInFleet.fuelCapacity.toLocaleString()} L capacity)
        </div>
      )}
...
function LegTable({
  ...
  const totals = legs.reduce(
    (a, l) => ({
      dist: a.dist + l.distanceKm,
      days: a.days + l.travelDays,
      fuel: a.fuel + l.fuelBurn
    }),
    { dist: 0, days: 0, fuel: 0 }
  )
  const cumDaysArr = legs.reduce<number[]>((acc, leg, i) => {
    acc.push((i > 0 ? acc[i - 1] : 0) + leg.travelDays)
    return acc
  }, [])
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
      <thead>
        <tr>
          ...
          <th style={th}>To</th>
          <th style={th}>Type</th>
          <th style={thR}>Distance</th>
          <th style={thR}>Time</th>
          <th style={thR}>Fuel</th>
          {gameDate && <th style={thR}>Arrival</th>}
        </tr>
      </thead>
      <tbody>
        {legs.map((leg, i) => {
          const cumDays = cumDaysArr[i]
          ...
          return (
            <tr key={i} style={{ opacity: leg.type === 'jump' ? 0.6 : 1 }}>
              ...
              <td style={td}>
                {leg.type === 'jump' ? (
                  <span style={{ color: '#bc8cff', fontWeight: 600 }}>JUMP</span>
                ) : (
                  <span style={{ color: 'var(--cic-cyan-dim)' }}>transit</span>
                )}
              </td>
              <td style={tdR}>{leg.type === 'jump' ? '—' : formatDistance(leg.distanceKm)}</td>
              <td style={tdR}>{leg.type === 'jump' ? '—' : `${leg.travelDays}d`}</td>
              <td style={tdR}>
                {leg.type === 'jump' ? '—' : `${leg.fuelBurn.toLocaleString()} L`}
              </td>
              {gameDate && (
                <td style={{ ...tdR, color: 'var(--cic-cyan-dim)' }}>
                  {leg.type === 'jump' ? '—' : arrival}
                </td>
              )}
            </tr>
          )
      ...
      <tfoot>
        <tr style={{ fontWeight: 600 }}>
          <td
            colSpan={3}
            style={{ padding: '3px 6px', borderTop: '2px solid var(--cic-panel-edge)' }}
          >
            Total
          </td>
          <td style={{ ...tdR, borderTop: '2px solid var(--cic-panel-edge)' }}>
            {formatDistance(Math.round(totals.dist))}
          </td>
          <td style={{ ...tdR, borderTop: '2px solid var(--cic-panel-edge)' }}>
            {Math.round(totals.days * 100) / 100}d
          </td>
          <td style={{ ...tdR, borderTop: '2px solid var(--cic-panel-edge)' }}>
            {Math.round(totals.fuel).toLocaleString()} L
          </td>
          {gameDate && (
            <td style={{ ...tdR, borderTop: '2px solid var(--cic-panel-edge)' }}>
              {addDaysToDate(gameDate, totals.days)}
            </td>
          )}
        </tr>
      </tfoot>
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
      <thead>
        <tr>
          ...
          <th style={thR}>Start</th>
          <th style={thR}>Burn</th>
          <th style={thR}>At Dest</th>
      ...
      <tbody>
        {lastTransitLeg.shipFuel.map((sf) => {
          ...
          const capacity =
            sf.fuelPct > 0
              ? Math.round((Math.max(0, sf.fuelRemaining) / sf.fuelPct) * 100)
              : startFuel
          ...
          return (
            <tr
              key={sf.shipId}
              style={{ background: isBottleneck ? 'rgba(255,23,68,0.08)' : undefined }}
            >
              <td
                style={{
                  ...td,
                  fontWeight: isBottleneck ? 600 : 400,
                  color: isBottleneck ? 'var(--cic-red)' : undefined
                }}
              >
                {sf.name}
              </td>
              ...
              <td
                style={{
                  ...tdR,
                  color: !sf.sufficient ? 'var(--cic-red)' : undefined,
                  fontWeight: !sf.sufficient ? 600 : 400
                }}
              >
                {sf.fuelRemaining.toLocaleString()} L
              </td>
              <td style={td}>
                <DualFuelBar
                  startPct={startPct}
                  endPct={sf.fuelPct}
                  insufficient={!sf.sufficient}
                />
              </td>
            </tr>
          )
